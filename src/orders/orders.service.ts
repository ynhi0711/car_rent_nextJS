...
import Coupon, { CouponEnum } from 'database/models/coupon';
import Order from 'database/models/order';
import { Sequelize } from 'sequelize';
import OrderStatus, { OrderStatusEnum } from 'database/models/order_status';
import Payment from 'database/models/payment';
import { CarStatus, Status } from 'database/models/status';
import { Price } from 'database/models/prices';
import PaymentStatus, { PaymentStatusEnum } from 'database/models/payment_status';
import PaymentMethod, { PaymentMethodEnum } from 'database/models/paymnet_method';
...
export class OrdersService {
  ...
  async calculatePrice(createOrderDto: CreateOrderDto) {
    ...
    let price = (car.price[car.price.length - 1])?.final_price;
    if (!price) {
      throw APIException.throwException(HttpStatus.NOT_FOUND, {
        title: `car_id ${createOrderDto.car_id} is not found`,
      });
  ...
  async checkCarAvailableToRent(createOrderDto: CreateOrderDto) {
    ...
    let car = await this.carRepository.findOne({ where: { id: createOrderDto.car_id }, })
    if (!car) {
      throw APIException.throwException(HttpStatus.NOT_FOUND, {
        title: `car_id ${createOrderDto.car_id} is not found`,
      });
    ...
    const currentCar = await this.orderRepository.findOne({ where: { car_id: createOrderDto.car_id } })
    if (!currentCar) {
      return true
    }
  ...
  async findAll(limit: number, offset: number) {
    const result = await this.orderRepository.findAndCountAll(
      { include: [Payment], limit: Number(limit), offset: Number(offset) }
    )
    return new PagingDto(result.rows, result.count, Number(limit), Number(offset))
  }
}
