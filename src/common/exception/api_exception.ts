  ...
  static throwException(
    ...
    switch (httpStatus) {
      case HttpStatus.BAD_REQUEST: {
        const error = this.error(options, httpStatus, BadRequestException.name);
      ...
      case HttpStatus.FORBIDDEN: {
        const error = this.error(options, httpStatus, ForbiddenException.name);
        throw new ForbiddenException(error);
      }
      case HttpStatus.NOT_FOUND: {
        const error = this.error(options, httpStatus, NotFoundException.name);
      ...
      case HttpStatus.CONFLICT: {
        const error = this.error(options, httpStatus, ConflictException.name);
